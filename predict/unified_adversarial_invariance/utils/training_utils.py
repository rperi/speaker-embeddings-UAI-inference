...
def disentanglement_loss(y_true, y_pred):
    embedding_dim = int(y_pred.shape[-1].value / 2)
...
def prepare_callbacks(models, model_names, optimizers,
                      weights_path, remote_weights_path, logs_dir, sync_frequency,
    ...
    checkpoint_cb = ModelsCheckpoint(
        ...
        save_weights_only=False, period=1, verbose=1,
    ...
    tensorboard_cb = TensorBoard(log_dir=logs_dir, write_batch_performance=True)
